    ...
    <form action="{{ route('site.busca') }}">
        <div class="input-field col s6 m3">
            <select id="finalidade" name="finalidade">
                ...
                <option value="Aluguel" {{ isset($dados['finalidade']) && $dados['finalidade']=='Aluguel' ? " selected"
                    : "" }}>Aluguel</option>
                <option value="Troca" {{ isset($dados['finalidade']) && $dados['finalidade']=='Troca' ? " selected" : ""
                    }}>Troca</option>
                <option value="Venda" {{ isset($dados['finalidade']) && $dados['finalidade']=='Venda' ? " selected" : ""
                    }}>Venda</option>
            </select>
        ...
        <div class="input-field col s6 m3">
            <select id="categoria" name="categoria_id">
                ...
                <option value="{{ $categoria->id }}" {{ isset($dados['categoria_id']) &&
                    $dados['categoria_id']==$categoria->id ? " selected" : "" }}>{{ $categoria->titulo }}</option>
                @endforeach
            </select>
        ...
        <div class="input-field col s12 m6">
            <select id="municipio" name="municipio_id">
                ...
                <option value="{{ $municipio->id }}" {{ isset($dados['municipio_id']) &&
                    $dados['municipio_id']==$municipio->id ? " selected" : "" }}>{{ $municipio->nome }} - {{
                    $municipio->sigla_uf }}</option>
                @endforeach
            </select>
        ...
        <div class="input-field col s12 m4">
            <select id="valor" name="valor">
                <option value="">Todas</option>
                <option value="1" {{ isset($dados['valor']) && $dados['valor']==1 ? " selected" : "" }}>Até R$ 500,00
                </option>
